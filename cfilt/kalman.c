...
#define V_ALLOC_ASSERT_(p, n) V_ALLOC_ASSERT(p, n, cfilt_kalman_filter_free, filt)
#define M_ALLOC_ASSERT_(p, n, m) M_ALLOC_ASSERT(p, n, m, cfilt_kalman_filter_free, filt)

int
cfilt_kalman_filter_alloc(cfilt_kalman_filter* filt, const size_t n, const size_t m, const size_t k)
{
...
{
    // x_ = Fx + Bu
    EXEC_ASSERT(gsl_blas_dgemv, CblasNoTrans, 1.0, filt->F, filt->x, 0.0, filt->x_);
    EXEC_ASSERT(gsl_blas_dgemv, CblasNoTrans, 1.0, filt->B, filt->u, 1.0, filt->x_);

    // P_ = FPF^T + Q
    EXEC_ASSERT(gsl_blas_dgemm, CblasNoTrans, CblasNoTrans, 1.0, filt->F, filt->P, 0.0, filt->_FP);
    EXEC_ASSERT(gsl_blas_dgemm, CblasNoTrans, CblasTrans, 1.0, filt->_FP, filt->F, 0.0, filt->P_);
...
cfilt_kalman_filter_update(cfilt_kalman_filter* filt)
{
    ...
    EXEC_ASSERT(gsl_blas_dgemm, CblasNoTrans, CblasTrans, 1.0, filt->P_, filt->H, 0.0, filt->_PH_T);
    EXEC_ASSERT(gsl_matrix_memcpy, filt->_PH_T_R, filt->R);
    EXEC_ASSERT(gsl_blas_dgemm, CblasNoTrans, CblasNoTrans, 1.0, filt->H, filt->_PH_T, 1.0, filt->_PH_T_R);
    EXEC_ASSERT(cfilt_matrix_invert, filt->_PH_T_R, filt->_inv, filt->_perm);
    EXEC_ASSERT(gsl_matrix_memcpy, filt->_PH_T_R, filt->_inv);
    EXEC_ASSERT(gsl_blas_dgemm, CblasNoTrans, CblasNoTrans, 1.0, filt->_PH_T, filt->_PH_T_R, 0.0, filt->K);

    // y = z - Hx_
    // y is used to avoid changing z
    EXEC_ASSERT(gsl_vector_memcpy, filt->y, filt->z);
    EXEC_ASSERT(gsl_blas_dgemv, CblasNoTrans, -1.0, filt->H, filt->x_, 1.0, filt->y);

    // x = x_ + Ky
    // x_ is copied over to x to avoid changing x_
    EXEC_ASSERT(gsl_vector_memcpy, filt->x, filt->x_);
    EXEC_ASSERT(gsl_blas_dgemv, CblasNoTrans, 1.0, filt->K, filt->y, 1.0, filt->x);

    // P = (I - KH)P_
    gsl_matrix_set_identity(filt->_I);
    EXEC_ASSERT(gsl_blas_dgemm, CblasNoTrans, CblasNoTrans, -1.0, filt->K, filt->H, 1.0, filt->_I);
    EXEC_ASSERT(gsl_blas_dgemm, CblasNoTrans, CblasNoTrans, 1.0, filt->_I, filt->P_, 0.0, filt->P);
